...
class FirebaseStorageAdapter implements ImageStorageAdapter {
  ...
  private async uploadViaSDK(buffer: Buffer, propertyId: string, imageIndex: number, contentType?: string): Promise<string> {
    ...
    try {
      ...
      // Generate public URL using Firebase Storage download format which works reliably with public rules
      // Format: https://firebasestorage.googleapis.com/v0/b/[bucket]/o/[path]?alt=media
      const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${this.bucket}/o/${encodeURIComponent(fileName)}?alt=media`;
  ...
  private async uploadViaRestAPI(buffer: Buffer, fileName: string, contentType?: string): Promise<string> {
    ...
    try {
      ...
      // Create public URL using Firebase Storage download format which works reliably with public rules
      // Format: https://firebasestorage.googleapis.com/v0/b/[bucket]/o/[path]?alt=media
      const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${this.bucket}/o/${encodeURIComponent(fullPath)}?alt=media`;
